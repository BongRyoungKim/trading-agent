...
class TradingEngine:
    ...
    # 저가 동전코인(원화 단가 1,000원 미만) 진입 제외.
    # 최초엔 100원 기준으로 도입했으나, 이후 누적된 거래를 가격 구간별로
    # 재분석한 결과 100~1,000원 구간도 여전히 손실 우위였다:
    #   100원 미만    : 승률 22.2% / profit_factor 0.73 / 순손익 -13,578원 (9건)
    #   100~1,000원   : 승률 40.0% / profit_factor 0.86 / 순손익  -2,593원 (5건)
    #   1,000원 이상  : 승률 61.1% / profit_factor 1.36 / 순손익  +9,592원 (18건)
    # 가격이 낮을수록 승률·손익이 함께 나빠지는 패턴이 뚜렷해 기준을
    # 1,000원으로 상향한다. 거래량 급증만으로 진입 조건을 충족하는 저가
    # 코인의 변동성·슬리피지 리스크를 걸러내기 위한 최소 단가 필터.
    _MIN_ENTRY_PRICE_KRW = Decimal("1000")
